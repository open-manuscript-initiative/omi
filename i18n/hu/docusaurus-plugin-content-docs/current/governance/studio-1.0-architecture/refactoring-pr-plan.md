...
| C15 | Hardenelje a webes kézbesítést és a lektorált/nem lektorált minősítést úgy, hogy a webhely ne váljon workflow-authority-vé; adjon külön DNS-hitelesített publication-venue autoritást domain-admin és szerkesztői szerepkörökkel. | web artifact service/UI, editorial decision és venue-authority service, identity/application Prisma migration, `/api/v1/publications/web`, venue authority API, WordPress/generic adapter, docs | C03–C05, C09–C11 | exact-revision/content evidence, DNS challenge/revalidation, domain-admin delegálás/utolsó admin védelme, editor grant/revoke/concurrency, hamis/hiányzó pecsét, anonimitás, approval grant, idempotency, timeout/retry/reconcile, accessibility, receiver contract | Preview protocol és DB bővítés | high |
