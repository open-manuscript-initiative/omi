---
title: OMI Terminology and Definitions
sidebar_label: Terminology and Definitions
sidebar_position: 50
---
...
| Field | Value |
|---|---|
| Document type | Governance terminology policy and central glossary |
| Status | Draft |
| Version | 0.1.0 |
| Normative language | English |
| Applies to | OMI specifications, schemas, profiles, registries, examples, implementations, and official translations |

## 1. Purpose

This document defines the shared terminology of the Open Manuscript Initiative (OMI).

Its purpose is to ensure that the same term has the same meaning across the complete OMI specification suite. It also separates concepts that are often treated as synonyms in word processors, publishing systems, repositories, citation managers, and editorial platforms.

The definitions in this document are intended to:

- provide a common conceptual vocabulary;
- prevent contradictory definitions between specifications;
- support precise schemas and APIs;
- improve interoperability between independent implementations;
- support consistent Hungarian, German, and future translations;
- make conformance requirements easier to interpret and test;
- distinguish semantic objects from their visual or serialized representations.

## 2. Authority and use

Terms with cross-specification meaning MUST use the definitions in this document.

An OMI specification MAY define a narrower specialised meaning for its own scope, but it MUST:

- identify the central term being specialised;
- state the narrower meaning explicitly;
- avoid contradicting the central definition;
- avoid silently reusing the term for a different concept.

When a term is not defined here, the applicable OMI specification governs. When neither this document nor an OMI specification defines a term, its ordinary technical meaning applies.

The English terms in this document are canonical for the normative English specification suite. Official translations MUST preserve the conceptual distinctions even where another language commonly uses one word for several English terms.

## 3. Normative language

The key words **MUST**, **MUST NOT**, **SHOULD**, **SHOULD NOT**, and **MAY** express normative requirement levels when written in uppercase.

Lowercase uses of words such as “must”, “should”, or “may” are ordinary prose and do not create independent conformance requirements.

## 4. Terminology principles

### 4.1 Concept before label

A term names a concept. The concept remains stable even when:

- a user interface uses a translated or shortened label;
- a filename changes;
- a visual rendering uses different wording;
- an external standard uses a different term;
- a discipline applies a local synonym.

### 4.2 Semantic identity before presentation

OMI distinguishes an object from its visual representation.

Examples:

- a citation occurrence is not the formatted citation text;
- a heading object is not a particular font size;
- an annotation is not a coloured highlight;
- a bibliographic record is not a bibliography entry string;
- a manuscript is not a DOCX or PDF file.

### 4.3 Local identity and external identity
