...
| Reference implementation | **Public beta (`0.2.0-beta.2`)** | Structured editing, multilingual UI/help, server-backed accounts, peer review, native OJS/OMP integration, imports/exports, publisher profiles, search, desktop/mobile builds, cross-platform update notifications and immutable release provenance are present. |
...
Tauri desktop builds are produced for Windows, Linux and macOS, with Android built from the same shared release line and an iOS/iPadOS simulator smoke target in CI. Cross-platform update notifications are implemented. Current release automation binds a release tag to the exact successful build commit and treats already published assets as immutable. Windows code-signing preparation is underway; unsigned or not-yet-reputation-established builds may still trigger platform warnings.
