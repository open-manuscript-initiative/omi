...
For a journal or press without OJS/OMP, publication-venue authority may additionally be verified by a one-time DNS TXT challenge under the venue's domain. DNS control is an **organizational authority proof, not peer-review evidence**. Successful DNS verification grants a `DOMAIN_ADMIN` role that may authorize existing Studio accounts as `EDITOR` or `EDITOR_IN_CHIEF`; only those editor roles, combined with manuscript-workspace editor access and a completed native scientific review round, may create a publisher-verified acceptance. The decision is also bound to the assurance-neutral publication-content digest and stores an immutable snapshot of venue, domain, DNS verification identity/time and editor role. DNS is periodically revalidated before new publisher-verified decisions; later DNS or membership changes do not rewrite historical decision provenance.
